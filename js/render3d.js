...
const GUN_SHAPE = {
  pistol:  { body: [11, 5.0, 3.6], barrel: [0.9,  5], mag: [2.6, 6, 2.6], grip: 0.55, stock: 0 },
...
const R3D = {
  ...
  init(canvas) {
    ...
    this.scene.add(sun);
    this.scene.add(new THREE.HemisphereLight(0xa8ccf5, 0x4a6038, 0.62));
    // Etwas Grundhelligkeit: seit dem Schattenwurf waren Unterseiten — etwa
    // eine Baumkrone ueber dem Kopf — pechschwarz statt nur dunkel.
    this.scene.add(new THREE.AmbientLight(0x8fa6c4, 0.16));
  ...
  buildDynamic() {
    ...
    const box = () => new THREE.BoxGeometry(1, 1, 1);
    /* Runde Grundkoerper statt Quader: der Rumpf verjuengt sich, Gliedmassen
       sind Zylinder und an jedem Gelenk sitzt eine Kugel, die den Uebergang
       verdeckt. Das ist der Unterschied zwischen Klotz und Figur. */
  ...
  buildViewModel() {
    ...
    V.guard = new THREE.Mesh(new THREE.BoxGeometry(4.0, 4.0, 10), dark);   // Handschutz
    V.charge = new THREE.Mesh(new THREE.BoxGeometry(1.4, 1.4, 5), dark);   // Ladegriff
    V.muzzle = new THREE.Mesh(new THREE.CylinderGeometry(1.6, 1.6, 3.4, 8), dark);
    V.muzzle.rotation.x = Math.PI / 2;
  ...
  applyViewWeapon(gun) {
    ...
    V.sight.visible = !S.scope;
    V.scope.position.set(0, bh * 0.62 + 2.4, -bl * 0.6);
    V.scope.scale.set(1, bl * 0.42 / 9, 1);
    V.guard.scale.set(bd / 4.0, bh / 5.4, Math.max(0.3, S.barrel[1] * 0.5) / 10);
    V.guard.position.set(0, bh * 0.05, -bl - S.barrel[1] * 0.24);
    V.charge.position.set(bd * 0.42, bh * 0.35, -bl * 0.72);
    V.muzzle.scale.set(S.barrel[0] / 1.6 * 1.35, 1, S.barrel[0] / 1.6 * 1.35);
    V.muzzle.position.set(0, bh * 0.16, -bl - S.barrel[1] - 1.2);
...
Object.assign(R3D, {
  ...
  drawGun(gi, id, rar, ox, oy, oz, fx, fz, tilt, upright) {
    ...
    // Schaft hinten, Griff darunter, Zielfernrohr nur wo es hingehoert
    const stockL = S.stock === 0 ? 0.001 : bl * 0.38;
    this.put(this.dyn.gunStock, gi, ox - fx * (bl * 0.5 + stockL * 0.5), oy - S.body[1] * 0.18,
             oz - fz * (bl * 0.5 + stockL * 0.5), S.body[2] * 0.85, S.body[1] * 0.85, stockL, qBody);
    this.put(this.dyn.gunGrip, gi, ox - fx * bl * 0.30, oy - S.body[1] * 0.5 - 3.4,
             oz - fz * bl * 0.30, S.body[2] * 0.8, 7.5, S.body[2] * 0.95, qBody);
    const scopeL = S.scope ? bl * 0.42 : 0.001;
    this.put(this.dyn.gunScope, gi, ox + fx * bl * 0.08, oy + S.body[1] * 0.62,
             oz + fz * bl * 0.08, 2.1, scopeL, 2.1, qBar);
  },
  ...
  updateActors(p) {
    ...
    for (const a of Game.actors) {
      ...
      this.put(D.hips, n, a.x, hipY, a.y, 23, H * 0.11, 17, Q, legCol);
      this.put(D.torso, n, a.x, chestY, a.y, 27, H * 0.27, 19, Q, C);
      this.put(D.neck, n, a.x, base + H * 0.78, a.y, 8, H * 0.06, 8, Q, hurt ? white : skin);
      this.put(D.head, n, a.x + c * 1.5, headY, a.y + sn * 1.5, 15, 16.5, 15, Q, hurt ? white : skin);
      // Muetze in Trikotfarbe: sonst verschwimmt der Hautton mit dem Rumpf
      this.put(D.cap, n, a.x + c * 1.5, headY + 7.5, a.y + sn * 1.5, 16, 6, 15.5, Q, C);
      this.put(D.brim, n, a.x + c * 8, headY + 5.2, a.y + sn * 8, 15, 2, 15, Q, C);
      ...
      for (let k = 0; k < 2; k++) {
        ...
        this.seg(D.limb, n * 8 + k * 2, hx, hipY, hz, kx, kY, kz, 9.5, legCol);
        this.seg(D.limb, n * 8 + k * 2 + 1, kx, kY, kz, ax2, aY, az2, 8.5, legCol);
        this.put(D.joint, n * 8 + k * 2, hx, hipY, hz, 10, 10, 10, Q, legCol);
        this.put(D.joint, n * 8 + k * 2 + 1, kx, kY, kz, 9, 9, 9, Q, legCol);
      ...
      for (let k = 0; k < 2; k++) {
        ...
        this.seg(D.limb, n * 8 + 4 + k * 2, sx, shY, sz, ex, eY, ez, 8.5, C);
        this.seg(D.limb, n * 8 + 4 + k * 2 + 1, ex, eY, ez, wx, wY, wz, 7.5, C);
        this.put(D.joint, n * 8 + 4 + k * 2, sx, shY, sz, 11, 11, 11, Q, C);
        this.put(D.joint, n * 8 + 4 + k * 2 + 1, ex, eY, ez, 8.5, 8.5, 8.5, Q, C);
        this.put(D.hand, n * 2 + k, wx, wY, wz, 7.5, 7.5, 7.5, Q, hurt ? white : skin);
    ...
    for (const k of ['torso', 'hips', 'head', 'cap', 'brim', 'neck', 'pack', 'shadow']) {
      D[k].count = n; D[k].instanceMatrix.needsUpdate = true;
      if (D[k].instanceColor) D[k].instanceColor.needsUpdate = true;
    }
    for (const k of ['limb', 'joint']) {
      D[k].count = n * 8; D[k].instanceMatrix.needsUpdate = true;
      if (D[k].instanceColor) D[k].instanceColor.needsUpdate = true;
    }
    ...
    for (const k of ['gunBody', 'gunBarrel', 'gunMag', 'gunStock', 'gunGrip', 'gunScope']) {
