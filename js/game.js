...
const Game = {
  ...
  /* ------------------------------------------------------ bullets
     Geschosse wurden frueher in Teilschritten von bis zu 26 Einheiten
     bewegt und nur an den Stuetzpunkten geprueft. Hauswaende sind aber nur
     20 Einheiten dick — je nachdem, wo man stand, passte die Wand genau
     zwischen zwei Stuetzpunkte und der Schuss ging hindurch. Deshalb jetzt
     ein exakter Streckentest: die gesamte Flugstrecke eines Bildes wird
     gegen die Geometrie geschnitten, unabhaengig von Schrittweite und
     Standposition.                                                        */

  /** Kleinstes t in [0,1], an dem die Strecke ein Rechteck betritt, sonst -1 */
  segBox(x0, y0, dx, dy, minx, miny, maxx, maxy) {
    let t0 = 0, t1 = 1;
    if (Math.abs(dx) < 1e-9) { if (x0 < minx || x0 > maxx) return -1; }
    else {
      let a = (minx - x0) / dx, bq = (maxx - x0) / dx;
      if (a > bq) { const h = a; a = bq; bq = h; }
      if (a > t0) t0 = a; if (bq < t1) t1 = bq;
      if (t0 > t1) return -1;
    }
    if (Math.abs(dy) < 1e-9) { if (y0 < miny || y0 > maxy) return -1; }
    else {
      let a = (miny - y0) / dy, bq = (maxy - y0) / dy;
      if (a > bq) { const h = a; a = bq; bq = h; }
      if (a > t0) t0 = a; if (bq < t1) t1 = bq;
      if (t0 > t1) return -1;
    }
  ...
  /** Kleinstes t in [0,1], an dem die Strecke einen Kreis betritt, sonst -1 */
  segCircle(x0, y0, dx, dy, cx, cy, r) {
  ...
  updateBullets(dt) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const travel = Math.min(sp * dt, b.left);
      if (travel <= 0.0001) { this.endBullet(b); continue; }
      const f = travel / sp;
      const dx = b.vx * f, dy = b.vy * f, dz = (b.vz || 0) * f;
      const x0 = b.x, y0 = b.y, z0 = b.y3;
      b.px = x0; b.py = y0;

      const hit = this.sweep(b, x0, y0, z0, dx, dy, dz);
      const t = hit ? hit.t : 1;
      b.x = x0 + dx * t; b.y = y0 + dy * t; b.y3 = z0 + dz * t;
      b.left -= travel * t;

      if (hit) { this.resolveHit(b, hit); continue; }
      if (b.left <= 0.0001 || b.x < 0 || b.y < 0 || b.x > MAP || b.y > MAP) this.endBullet(b);
    }
  },

  /** Frueheste Kollision der Strecke; null wenn frei */
  sweep(b, x0, y0, z0, dx, dy, dz) {
    let best = null;
    const mx = x0 + dx / 2, my = y0 + dy / 2;
    const rad = Math.hypot(dx, dy) / 2 + 70;
    const zAt = t => z0 + dz * t;

    // --- Boden
    if (dz < 0 && z0 + dz <= 0) {
      const t = clamp(-z0 / dz, 0, 1);
      best = { t, kind: 'ground' };
    }

    // --- Akteure
    const acts = this.aGrid.query(mx, my, rad, this._tmp);
    for (let i = 0; i < acts.length; i++) {
      ...
      const z = zAt(t);
      if (z < 0 || z > BODY_H) continue;
      best = { t, kind: 'actor', obj: o };
    }

    // --- gebaute Waende
    const built = World.bGrid.query(mx, my, rad, this._q2);
    for (let i = 0; i < built.length; i++) {
      const s = built[i];
      if (s.hp <= 0) continue;
      const t = this.segBox(x0, y0, dx, dy, s.bx, s.by, s.bx2, s.by2);
      if (t < 0 || (best && t >= best.t)) continue;
      const grow = s.buildT > 0 ? clamp(1 - s.buildT / s.build, .15, 1) : 1;
      if (zAt(t) > BUILT_H * grow) continue;
      best = { t, kind: 'built', obj: s };
    }

    // --- Daecher: ueber die Wand hinweg ins Haus schiessen ging bisher,
    //     obwohl dort sichtbar ein Dach liegt. Der Grundriss ist eine
    //     waagerechte Ebene auf Wandhoehe.
    if (Math.abs(dz) > 1e-9) {
      const tr = (ROOF_Y - z0) / dz;
      if (tr >= 0 && tr <= 1 && (!best || tr < best.t)) {
        const rx = x0 + dx * tr, ry = y0 + dy * tr;
        const foot = World.sGrid.query(rx, ry, 40, this._q3 || (this._q3 = []));
        for (let i = 0; i < foot.length; i++) {
          const o = foot[i];
          if (o.w === undefined || o.solid) continue;          // nur Grundrisse
          if (rx < o.x || rx > o.x + o.w || ry < o.y || ry > o.y + o.h) continue;
          best = { t: tr, kind: 'static', obj: o };
          break;
        }
      }
    }

    // --- Hauswaende, Baeume, Felsen
    const st = World.sGrid.query(mx, my, rad, this._q);
    for (let i = 0; i < st.length; i++) {
      const o = st[i];
      let t;
      if (o.w !== undefined) {
        if (!o.solid) continue;
        t = this.segBox(x0, y0, dx, dy, o.x, o.y, o.x2, o.y2);
        if (t < 0 || (best && t >= best.t)) continue;
        if (zAt(t) > WALL_H) continue;
      } else {
        if (o.hp <= 0 || o.type === 'crate') continue;
        t = this.segCircle(x0, y0, dx, dy, o.x, o.y, o.r * 0.62);
        if (t < 0 || (best && t >= best.t)) continue;
        const z = zAt(t);
        if (o.type === 'tree' && z > 150 * o.s) continue;      // Krone haengt hoeher
        if (o.type === 'rock' && z > o.r * 0.95) continue;     // Findlinge sind kniehoch
      }
      best = { t, kind: 'static', obj: o };
    }
    return best;
  },

  resolveHit(b, hit) {
    if (b.rocket) { this.explode(b.x, b.y, b); return; }
    switch (hit.kind) {
      case 'ground':
        this.spark3(b.x, 4, b.y, 5, .2);
        this.spark(b.x, b.y, rnd(TAU), 90, '#c8bda0', .18);
        this.endBullet(b);
        break;
      case 'actor': {
        const o = hit.obj;
      ...
      case 'built':
        World.damageStructure(hit.obj, b.dmg);
        this.spark(b.x, b.y, Math.atan2(-b.vy, -b.vx), 160, MAT_INFO[hit.obj.mat].col, 0.2);
        this.spark3(b.x, b.y3, b.y, 5, .18);
        this.endBullet(b);
        break;
      default: {
        const o = hit.obj;
        if (o.w === undefined) o.hp -= b.dmg * 0.5;
